package com.tianzhili.www.myselfsdk.SuspensionIndexBar.suspension;
...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        ...
        int baseline = ( rect.top + rect.bottom - fontMetrics.bottom - fontMetrics.top) / 2;
...
//        c.drawText(mDatas.get(position).getSuspensionTag(), child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
        ...
        c.drawText(mDatas.get(position).getSuspensionTag(),child.getPaddingLeft(), baseline,mPaint);
    }
