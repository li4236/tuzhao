...
import com.tianzhili.www.myselfsdk.dashboardview.Interpolator.SpringInterpolator;
import com.tianzhili.www.myselfsdk.dashboardview.util.PxUtils;
import com.tianzhili.www.myselfsdk.dashboardview.util.StringUtil;
...
//        drawerPointer(canvas, percent);
    ...
    private void drawText(Canvas canvas, float percent) {
        ...
        float length;
        paintText.setTextSize(mTextSize / 3);
...
//                canvas.drawLine(0, numY + 5, 0, numY + 25, paintNum);//画长刻度线
                if (tikeStrArray.length > (i % tikeGroup)) {
                    ...
                    int baseline = ((numY + 10) + (fontMetrics.bottom - fontMetrics.top) / 2);
...
//        canvas.drawCircle(0, 0, mWidth / 2 - 2, paintOutCircle);
    ...
    private void startAnimator(float start, float end, long animTime) {
        valueAnimator = ValueAnimator.ofFloat(start, end).setDuration(animTime);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                DashboardView.this.percent = (float) animation.getAnimatedValue();
