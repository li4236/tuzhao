package com.tianzhili.www.myselfsdk.filter.adapter;
...
import com.tianzhili.www.myselfsdk.filter.FilterTypeBean;
import com.tianzhili.www.myselfsdk.filter.demobean.AreaBean;
import com.tianzhili.www.myselfsdk.filter.demobean.CityBean;
...
import com.tianzhili.www.myselfsdk.filter.demobean.ProvinceBean;
import com.tianzhili.www.myselfsdk.filter.interfaces.OnFilterDoneListener;
import com.tianzhili.www.myselfsdk.filter.typeview.DoubleGridView;
import com.tianzhili.www.myselfsdk.filter.typeview.DoubleListView;
import com.tianzhili.www.myselfsdk.filter.typeview.MultiGridView;
import com.tianzhili.www.myselfsdk.filter.typeview.SingleGridView;
import com.tianzhili.www.myselfsdk.filter.typeview.SingleListView;
import com.tianzhili.www.myselfsdk.filter.typeview.ThreeListView;
import com.tianzhili.www.myselfsdk.filter.util.CommonUtil;
import com.tianzhili.www.myselfsdk.filter.util.DpUtils;
import com.tianzhili.www.myselfsdk.filter.util.FilterUtils;
...
public class DropMenuAdapter implements MenuAdapter {
    ...
    private List<InstitutionPriceBean> sortListData;//排序 数据源
    private List<InstitutionPriceBean> kindListData;//类别 数据源
    private List<InstitutionPriceBean> distanceListData;//距离 数据源
    ...
    public View getView(int position, FrameLayout parentContainer) {
        ...
        switch (position) {
            case 0://排序
                view = createSingleListView(sortListData,position,0);
                break;
            case 1://类别
                view = createSingleListView(kindListData,position,0);
                break;
            case 2://距离
                view = createSingleListView(distanceListData,position,2);
                break;
//            case 3://多筛选
//                view = createMutiGrid(position);
...
//                break;
    ...
    public void setSortListData(List<InstitutionPriceBean> sortListData) {
        this.sortListData = sortListData;
    }

    public void setKindListData(List<InstitutionPriceBean> kindListData) {
        this.kindListData = kindListData;
    }

    public void setDistanceListData(List<InstitutionPriceBean> distanceListData) {
        this.distanceListData = distanceListData;
    }
    ...
    public interface OnSortCallbackListener {
        void onSortCallbackListener(int item,int position);
    }
     ...
     * 02 多筛选设置（5个grid数据）
     */
    private View createMutiGrid(final int tabPosition) {
     ...
     * @return
     */
    private View createSingleListView(List<InstitutionPriceBean> beanList, final int tabPosition,int checkPosition) {

        SingleListView<InstitutionPriceBean> singleListView = new SingleListView<InstitutionPriceBean>(mContext)
                .setSingleListAdapter(new SimpleTextAdapter<InstitutionPriceBean>(null, mContext) {
                    @Override
                    public String provideText(InstitutionPriceBean bean) {
                        return bean.getName();
                    }
                ...
                .onSingleListClick(new SingleListView.OnSingleListClickListener<InstitutionPriceBean>() {
                    @Override
                    public void onSingleListCallback(InstitutionPriceBean item) {
                        FilterUtils.instance().positionTitle = item.getName();
                        onSortCallbackListener.onSortCallbackListener(item.getId(),tabPosition);//回调到act中操作
                        onFilterDone(tabPosition);
                    }
                });

        //设置默认选项
        singleListView.setList(beanList, checkPosition);
    ...
    private void onFilterDone(int tabPosition) {
        if (onFilterDoneListener != null) {
            if (tabPosition == 0) {
                ...
                onFilterDoneListener.onFilterDone(3, "筛选", "");
            } else {
