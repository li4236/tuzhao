package com.tuzhao.publicwidget.timer;
...
public class CountupView extends View {
  ...
  private long mRemainTime,thetime = 0;
  ...
  public CountupView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mTimeTextWidth = rect.width()+(int) (rect.width()*0.4);
...
//    }
    ...
    if (mInterval > 0 && null != mOnCountdownIntervalListener) {
      if (thetime == 0){
        thetime = mRemainTime;
        mOnCountdownIntervalListener.onInterval(this, mRemainTime);
      }else if (mRemainTime - thetime >= mInterval){
        thetime = mRemainTime;
        mOnCountdownIntervalListener.onInterval(this, mRemainTime);
      }
//      if (mPreviouIntervalCallbackTime == 0) {
